...
void NoximRouter::rxProcess()
{
	...
	for( i=0; i<4; i++){
		if( DW_tag_neighbor[i].read() < DW_tag_cur && DW_tag_neighbor[i].read() >= 0){
					//DW level propagation: 選擇鄰居或自己的DW_tag值較小的那個																																								 //做為新的DW level (選大的可能會NW saturation)
...
vector<int> NoximRouter::routingDownward_CrossLayer(const NoximCoord& current, const NoximCoord& destination, const NoximCoord& source)
{
	...
	int layer = 3;	//表示此packet應該在哪個layer做XY傳遞
	   ...
	   if(current.z < layer && (current.x != destination.x || current.y != destination.y) )  
		...
		{
			 directions = routingXYZ(current, destination);	 //Matthew:  directions = routingXYZ(current, destination)		                                                         //原本downward routing用XY 這裡用westfirst
		}
		else if((current.x == destination.x && current.y == destination.y) && current.z > destination.z)	//xy相同, z方向往上傳
		{	
			directions.push_back(DIRECTION_UP);
